...
serve(async (req) => {
  ...
  try {
    console.log('Received sync-aromes request');
    ...
    console.log(`Processing ${products.length} arome(s)`);
    ...
    if (products.length === 0 || (keys0.length === 1 && keys0[0] === '')) {
      return new Response(JSON.stringify({
        ...
        hint: "Dans le node HTTP Request, mets Body Content Type = JSON et Body = {{$json}} (mode RAW/JSON).",
      }), {
        status: 400,
    ...
    // Mapping spécifique pour les arômes
    const mappedProducts = products.map((p: any) => {
      ...
      return {
        ...
        typologie_de_produit: get(p, ["Typologie de produit", "typologie_de_produit", "typologie", "Typologie"]) || 'AROME',
    ...
    if (nonEmpty.length === 0) {
      return new Response(JSON.stringify({
        ...
        hint: "Vérifie que les en-têtes Google Sheets correspondent.",
      }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    ...
    console.log(`Aromes actifs/inactifs: ${activeProducts.length}, Aromes marqués SUPPRIME: ${deletedProducts.length}`);
    console.log('Sample mapped arome:', JSON.stringify(nonEmpty[0]));
    ...
    if (activeProducts.length > 0) {
      ...
      console.log(`Upsert réussi: ${upsertedCount} arome(s)`);
    }
    ...
    if (deletedProducts.length > 0) {
      ...
      if (deleteError) {
        ...
        console.log(`Supprimé ${deletedCount} arome(s)`);
      }
    }
    ...
    return new Response(JSON.stringify({ 
      ...
      message: `${upsertedCount} arome(s) mis à jour, ${deletedCount} supprimé(s)`
    }), { 
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
    ...
    console.error('Error in sync-aromes:', errorMessage);
    return new Response(JSON.stringify({ 
      success: false,
      error: errorMessage 
    }), {
