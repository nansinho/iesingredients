  ...
  return (
    <Link to={`/${lang}/produit/${product.code}`} className="group block">
      ...
      >
        ...
        <div className="relative h-24 sm:h-28 overflow-hidden bg-secondary/30">
          <img 
            src={config.image} 
          ...
          {product.gamme && (
            <div className="absolute top-2 sm:top-3 right-2 sm:right-3">
              <Badge className={cn("text-[9px] sm:text-[10px] font-semibold px-1.5 sm:px-2 py-0.5 border-0 shadow-sm", config.badge)}>
                {product.gamme}
              </Badge>
            </div>
          )}
          ...
          <div className="absolute -bottom-5 sm:-bottom-6 left-3 sm:left-4">
            <div className={cn(
              "w-12 h-12 sm:w-14 sm:h-14 rounded-xl flex items-center justify-center font-serif text-base sm:text-lg font-bold border-4 border-card shadow-lg",
        ...
        <div className="p-3 sm:p-5 pt-7 sm:pt-8 space-y-2 sm:space-y-3">
          {/* Title & Code */}
          <div>
            <h3 className="font-serif text-base sm:text-lg font-semibold text-foreground leading-snug line-clamp-2 group-hover:text-primary transition-colors">
              {product.nom_commercial || 'Produit sans nom'}
            </h3>
            {/* Clickable code - more prominent */}
            <div className="mt-1.5 flex items-center gap-1.5">
              <span className="font-mono text-sm sm:text-base font-semibold text-primary">{product.code}</span>
              <CopyField 
                label="" 
                value={product.code || ''} 
                mono 
          ...
          {/* INCI - Clickable */}
          {product.inci && (
            <div className="bg-secondary/30 px-2 py-1.5 rounded overflow-hidden">
              <CopyField 
                label="INCI" 
                value={product.inci} 
                mono 
                className="text-[10px] sm:text-xs text-muted-foreground break-all line-clamp-2"
          ...
          {product.aspect && (
            <Badge variant="outline" className="text-[9px] sm:text-[10px] font-medium px-1.5 sm:px-2 py-0.5 bg-secondary/50">
              {product.aspect}
            </Badge>
          )}
          ...
          <div className="grid grid-cols-2 gap-1.5 sm:gap-2 text-[10px] sm:text-xs">
            {product.origine && (
              <div className="flex items-center gap-1 sm:gap-1.5 text-muted-foreground overflow-hidden">
        ...
        <div className="absolute bottom-3 sm:bottom-4 right-3 sm:right-4 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
          <div className={cn("w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center", config.bar)}>
